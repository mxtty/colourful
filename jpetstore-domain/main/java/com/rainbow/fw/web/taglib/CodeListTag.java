 ...
 * 
 * <p>
 * サーブレットコンテキストから 属性 id で指定された CodeListLoader を探し出し、 その中に保存されているコードリストを取得する。
 * 見つからない場合は空のコードリストの取得を行う。
 * </p>
 * 
 * コードリスト内の要素はプロパティ名が id 、 name で格納されているため、アクセスするためには、タグ内のプロパティに これら要素名を用いる。
 * コードリストの読み込みは、 {@link jp.terasoluna.fw.web.codelist.CodeListLoader}
 * インタフェースの実装クラスを参照のこと。
 * 
 * 
 * <strong>タグがサポートする属性</strong><br>
 * <p>
 * defineCodeList タグでは、以下の属性をサポートする。
 * </p>
 * <div align="center">
 ...
 * インタフェースを実装したbeanの名前を指定する。 このタグ宣言以降、&lt;logic:iterator&gt;タグ、
 * &lt;html:options&gt;タグなどでコードリストが参照できる。
 * </tr>
 * </table>
 * </div> </p> <strong>カスタムタグのスクリプティング変数</strong><br>
 * <p>
 ...
 * 定義の方法は {@link jp.terasoluna.fw.web.codelist.DBCodeListLoader}、 及び
 * {@link jp.terasoluna.fw.web.codelist.MappedCodeListLoader} を参照。
 * </p>
 * 
 ...
 *  &lt;/html:select&gt;
 *  …
 * </pre></code> コードリストのサイズ取得に関しては、{@link WriteCodeCountTag} を参照 </p>
 * 
...
public class CodeListTag extends TagSupport {
	 ...
	 * 
	 * <p>
	 * サーブレットコンテキストから、ApplicationContextを取得し、 &quot;page&quot; 属性で指定された id で
	 * CodeListLoader を取得し、その中の コードリストを pageContext に登録する。
	 * 
	 * コードリストが発見できない場合、空のArrayListを pageContext に登録する。 なお、登録時のスコープは
	 * &quot;page&quot; 属性とする。
	 * </p>
	 * 
